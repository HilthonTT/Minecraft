...
public sealed class ScreenFBO
{
    ...
    private void CreateFBO(int screenWidth, int screenHeight)
    {
        ...
        GL.Ext.GenRenderbuffers(1, out _renderBuffer);
        GL.Ext.BindRenderbuffer(RenderbufferTarget.RenderbufferExt, _renderBuffer);
    ...
    {
        var code = GL.Ext.CheckFramebufferStatus(FramebufferTarget.FramebufferExt);
        switch (code)
        {
